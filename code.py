...
moPixelRow = neopixel.NeoPixel(26, mnSettingLEDCount, brightness=0.3, auto_write=False)
...
oBeamSet = neopixel.NeoPixel(27, mnBeamLEDCount, brightness=0.5, auto_write=True)
...
def UpdateSetting():
    ...
    for nIterator in range(mnSettingLEDCount - 1):
        if nIterator > 7:
            ...
            if mnIntensitySetting > 7:
                # if mnIntensitySetting >= nIterator:
                nRed = int((moRGBStrength - 7) * 16)
                nGreen = int(moRGBStrength - (4 * (mnIntensitySetting - 7)))
...
btn1 = Debouncer(ButtonRead(board.D5))
btn2 = Debouncer(ButtonRead(board.D6))
