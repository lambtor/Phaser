...
def UpdateIntensity():
    ...
    nIndxSettingMax = mnSettingLEDMax - 2
    nReversedPolarity = 0
    ...
    if (IS_TYPE_ONE_PHASER is True):
        ...
        for nIterator2 in range(mnSettingLEDMax - 1):
            ...
            if nIterator2 < mnIntensitySetting:
                ...
                nGreen = 128 - nRed
                # use nIterator2 instead for settings row leds that are aligned correctly
                # moSettingRow[nIterator2] = (nRed, nGreen, 0)
                # this is for LEDs wired in the wrong direction, 7-0 instead of 0-7
                nReversedPolarity = nIndxSettingMax - nIterator2
                moSettingRow[nReversedPolarity] = (nRed, nGreen, 0)
            else:
                # use nIterator2 instead for settings row leds that are aligned correctly
                nReversedPolarity = nIndxSettingMax - nIterator2
                # moSettingRow[nIterator2] = moRGBBlack
                moSettingRow[nReversedPolarity] = moRGBBlack
    else:
...
def WarningShotMode():
    ...
    global mnSettingLEDMax
    nIndxSettingMax = mnSettingLEDMax - 2
    # this is for reversed LEDS - set 7 as pink instead of 0
    moSettingRow.fill(moRGBBlack)
    moSettingRow[nIndxSettingMax] = moRGBWarn
    moSettingRow.write()
