...
def RunOverload():
    ...
    nNow = time.monotonic()
    if (nNow - mdecOverLastTime > mnOverFrameSpeed) and (
        mnCurrOverFrame <= mnMaxOverFrame
    ):
...
def UpdateMenuSetting():
    ...
    if mnMenuIndex == MENUIDX_EXIT:
        ...
        AnimateSettingChange(
            MenuOptions.Frequency[nFreq], MenuOptions.Frequency[moUser.Frequency]
        )
        ...
        if nTimeout < (len(MenuOptions.SleepTimer) - 1):
            ...
            moUser.SleepTimer = 0
        AnimateSettingChange(
            MenuOptions.SleepTimer[nTimeout], MenuOptions.SleepTimer[moUser.SleepTimer]
        )
        ...
        AnimateSettingChange(
            MenuOptions.Volume[nCurrVol], MenuOptions.Volume[moUser.Volume]
        )
        # Save setting to carry between sleep / wake
        ...
        AnimateSettingChange(
            MenuOptions.BeamBrightness[nBeam],
            MenuOptions.BeamBrightness[moUser.BeamBrightIndex],
        )
        ...
        moSettingRow.brightness = GetSettingBrightnessLevel()
        AnimateSettingChange(
            MenuOptions.SettingBrightness[nSetting],
            MenuOptions.SettingBrightness[moUser.SettingBrightIndex],
        )
...
while True:
    ...
    if (
        moUser.SleepTimer == 1
        and not any(moActiveMode == y for y in (2, 3))
        and (time.monotonic() - mdecBtnTime) > mnModeInterval
    ):
        CheckSleep()
